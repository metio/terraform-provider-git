...
func (r *dataSourceGitLogType) GetSchema(_ context.Context) (tfsdk.Schema, diag.Diagnostics) {
	return tfsdk.Schema{
		...
		Attributes: map[string]tfsdk.Attribute{
			...
			"all": {
				MarkdownDescription: "Pretend as if all the refs in `refs/`, along with `HEAD`, are listed. It is equivalent to running `git log --all`. If set on true, the `from` attribute will be ignored.",
				Type:                types.BoolType,
				Computed:            true,
				Optional:            true,
				PlanModifiers: []tfsdk.AttributePlanModifier{
...
func (r *dataSourceGitLog) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "Read git_log data source")
